...
public class SegmentSerializer {
    ...
    public byte[] serialize(Map<String, Object> map) {
        try (ByteArrayOutputStream baos = new ByteArrayOutputStream(); final Output output = new Output(baos, 8192)) {
            ...
            log.error("序列化失败", e);
            throw new EngineException("序列化失败");
        }
    }
    ...
    public Map<String, Object> deserialize(byte[] bytes) {
        ...
        try (ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream(bytes); Input input = new Input(byteArrayInputStream, 8192)) {
            Kryo kryo = kryoThreadLocal.get();
            ...
            log.error("反序列化失败", e);
            throw new EngineException("反序列化失败");
        }
    }
}
